...
`CLAUDE.md` and the eight prompt templates under `commands/` consume the same version-pinned CLI JSON and canonical Playbooks as the Codex adapter. `adapter-contract.json` declares the shared report version, command templates, consumed fields, and advisory classification labels used by cross-adapter tests.
...
`change-scope-review` explains the RG008 `scopeFindings` and `taskDrift` fields from that report. It stops for reported errors, pauses for user confirmation on warnings or medium/high drift, and does not calculate paths, budgets, or drift scores.
