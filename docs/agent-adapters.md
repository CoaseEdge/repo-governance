...
Codex releases install the seven self-contained Skills into `${CODEX_HOME:-$HOME/.codex}/skills`. Claude Code assets remain version-locked under the installed engine:
...
Codex release 会把七个自包含 Skill 安装到 `${CODEX_HOME:-$HOME/.codex}/skills`。Claude Code 的 `CLAUDE.md`、命令模板、可选 Hook 模板和两边共享的 Playbook 则保存在引擎目录的 `agent-assets/` 下，并由同一个 release manifest 摘要与 `engineCommitSha` 锁定。
